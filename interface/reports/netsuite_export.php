...
if ($_POST['form_orderby']) {
  ...
  $query = "( " .
    ...
    ******************************************************************/
    ...
    "(ap.code_type = '' OR (ap.code_type = b.code_type AND ap.code = b.code)) AND " .
    "(ap.pay_amount != 0.00) " .
    "ORDER BY ap.post_date DESC LIMIT 1), '') AS paydate " .
    ...
    ******************************************************************/
    ...
    "COALESCE((SELECT ap.post_date FROM ar_activity AS ap WHERE " .
    "ap.pid = fe.pid AND ap.encounter = fe.encounter AND ap.deleted IS NULL AND " .
    "(ap.code_type = '' OR (ap.code_type = 'PROD' AND ap.code = s.drug_id)) AND " .
    "(ap.pay_amount != 0.00) " .
    "ORDER BY ap.post_date DESC LIMIT 1), '') AS paydate " .
