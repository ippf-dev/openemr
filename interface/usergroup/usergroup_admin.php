  ...
  foreach ($olduf as $key => $value) {
    if ($value && ($i = strpos($key, '/')) !== false) {
      ...
      sqlStatement("DELETE FROM users_facility WHERE " .
        ...
        array('users', $user_id, $facid, $whid));
        ********************************************************/
        "tablename = 'users' AND table_id = " . add_escape_custom($user_id) .
        " AND facility_id = '" . add_escape_custom($facid) . "' AND warehouse_id = '" .
        add_escape_custom($whid) . "'");
    }
  }
}
...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] =="user_admin") {
    if ($_POST["mode"] == "update") {
      if (isset($_POST["rumple"])) {
        ...
        $user_data = sqlQuery("select * from users where id = ?", array($_POST["id"]));
...
<?php
if($set_active_msg == 1){
echo "<font class='alert'>".xlt('Emergency Login ACL is chosen. The user is still in active state, please de-activate the user and activate the same when required during emergency situations. Visit Administration->Users for activation or de-activation.')."</font><br>";
}
if ($show_message == 1){
 echo "<font class='alert'>" . xlt('The following Emergency Login User is activated:') . " <b>" . text($_GET['fname']) . "</b></font><br>";
 echo "<font class='alert'>" .
  xlt('Emergency Login activation email will be circulated only if following settings in the interface/globals.php file are configured:') .
  text(" \$GLOBALS['Emergency_Login_email'], \$GLOBALS['Emergency_Login_email_id']") . "</font>";
}
