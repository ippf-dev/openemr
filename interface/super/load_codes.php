  ...
  if (is_uploaded_file($tmp_name) && $_FILES['form_file']['size']) {
    ...
    if ($zipin->open($tmp_name) === true) {
      ...
      for ($i = 0; $i < $zipin->numFiles; ++$i) {
        ...
        if (
          $code_type == 'RXCUI' && basename($ename) == 'RXNCONSO.RRF' ||
          $code_type == 'ICD10' && preg_match('/^icd10cm_order_.*\.txt$/', basename($ename))
        ) {
    ...
    while (($line = fgets($eres)) !== false) {
      ...
      if ($code_type == 'RXCUI') {
        ...
        if (strlen($line) < 77) continue;
        $order           = trim(substr($line,  0,  5));
        $code            = trim(substr($line,  6,  7));
        if (strlen($code) > 3) $code = substr($code, 0, 3) . '.' . substr($code, 3);
        $submittable     = trim(substr($line, 14,  1));
        $code_text_short = trim(substr($line, 16, 60));
        $code_text       = trim(substr($line, 77    ));
        if ($submittable == '0') continue;
        if (isset($seen_codes[$code])) continue;
        $seen_codes[$code] = 1;
        if (!$form_replace) {
          $tmp = sqlQuery("SELECT id FROM codes WHERE code_type = ? AND code = ? LIMIT 1",
            array($code_type_id, $code));
          if ($tmp['count']) {
            sqlStatement("UPDATE codes SET code_text = ?, code_text_short = ? WHERE code_type = ? AND code = ?",
              array($code_text, '', $code_type_id, $code));
            ++$repcount;
            continue;
          }
        }
        sqlStatement("INSERT INTO codes SET code_type = ?, code = ?, code_text = ?, " .
          "code_text_short = ?, fee = 0, units = 0",
          array($code_type_id, $code, $code_text, ''));
        ++$inscount;
      }
...
foreach (array(
  ...
  echo "    <option value='$codetype'>$name</option>\n";
}
?>
   </select>
  </td>
...
<p class='text'>
<b>ICD-10-CM (diagnosis) codes</b> may be downloaded from
<a href='https://www.cms.gov/Medicare/Coding/ICD10/index.html' target='_blank'>
https://www.cms.gov/Medicare/Coding/ICD10/index.html</a>.
Click "20xx ICD-10-CM and GEMs" for the desired year and then download
"20xx Code Descriptions in Tabular Order". Then upload that file as-is here.
</p>
